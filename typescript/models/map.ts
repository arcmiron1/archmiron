...
class MapScene extends GameScene  {
     ...
     */
    propagateNoise(cmap:Array<Array<number>>, tries: number = 1, densityPercent: number = 80) {
        if (cmap == undefined) return false;
        if (cmap.length == 0) return false;
        if (cmap[0].length == 0) return false;

        // start of logic
        var rmap:any = [], ox, oy, nx, ny;
        ox = cmap.length;
        oy = cmap[0].length;
        ...
        for (let x = 0; x < ox; x++) {
            for (let y = 0; y < oy; y++) {
                ...
                ny = y * 2;
                let cval = cmap[x][y];
                if (rmap[nx] == undefined) rmap[nx] = [];
                if (rmap[nx+1] == undefined) rmap[nx+1] = [];
                [
                    rmap[nx][ny], 
                    rmap[nx+1][ny], 
                    rmap[nx][ny+1], 
        ...
        tries -=1;
        if (tries > 0) rmap = this.propagateNoise(rmap, tries);
    ...
    getNeighbors(uX: number, uY: number) {
    ...
    getStar(uX: number, uY: number) {
        ...
        if (neighbor[4] == 0) wallCounter |= (1<<8);
        if (neighbor[0] == 0) wallCounter |= (1<<7);
        if (neighbor[6] == 0) wallCounter |= (1<<6);
    
        if (neighbor[2] == 0) wallCounter |= (1<<5);
        if (neighbor[8] == 0) wallCounter |= (1<<4);
        if (neighbor[3] == 0) wallCounter |= (1<<3);
    
        if (neighbor[5] == 0) wallCounter |= (1<<2);
        if (neighbor[1] == 0) wallCounter |= (1<<1);
        if (neighbor[7] == 0) wallCounter |= 1;
        ...
        for (let p = 0; p < noiseLoop; p++) {
            ...
            for (let x = 0; x < w; x++) { // width
                for (let y = 0; y < h; y++) { // height
                    ...
                    if (this.noiseMap[uX][uY] == 1) continue;
    ...
    countWalls(neighbor:Array<number>) {
        let wallCounter = 0;
        for (const n in neighbor) {
            // console.log('each neighbor: ', n);
            if (neighbor[n] == 0) wallCounter++;
